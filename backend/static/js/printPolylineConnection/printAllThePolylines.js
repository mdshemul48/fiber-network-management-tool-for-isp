...
const printAllThePolylines = async () => {
  ...
  window.allTheConnection.forEach((connection, index) => {
    if (connection.type === "pointToPoint") printPointToPointConnection(connection, map, index);
    else if (connection.type === "corporate") printCorporateConnection(connection, map, index);
    else if (connection.type === "reseller") printResellerConnection(connection, map, index);
    else if (connection.type === "localFiber") printLocalFiberConnection(connection, map, index);
    else if (connection.type === "splitter") printLocalSplitter(connection, map, index);
    else if (connection.type === "home") printHomeConnection(connection, map, index);
  });
};
